function varargout=sml1_imana_new(what,varargin)
...
switch(what)
    ...
    case 'BEH_scanner_er_mt'                                                % Analyse and plot ER and MT per subject
        ...
        for i=1:5
            subplot(1,5,i)
            barplot(B_fun.ScanSess,B_fun.MT,'split',B_fun.seqType,'subset',B_fun.sn,'leg',{'train','untrain'})
    ...
    figure
            ...
            for s = sn
                ...
                R.blockNum=[1:length(R.timeTrial)]';
    ...
    case 'PREP_process1_func'                                               
        ...
        for s = sn
            sml1_imana_new('PREP_make_4dNifti','sn',s,'sessN',sessN);
            sml1_imana_new('PREP_makefieldmap','sn',s,'sessN',sessN);
            sml1_imana_new('PREP_make_realign_unwarp','sn',s,'sessN',sessN);
            sml1_imana_new('PREP_move_data','sn',s,'sessN',sessN);
            sml1_imana_new('PREP_meanimage_bias_correction','sn',s);
        end
    ...
    case 'pattern_reliability'
        ...
        figsess = 0;
        sessN = [1:4];
        subtract_partition_mean = 1; % subtract
        ...
        vararginoptions(varargin,{'roi','figsess','sn','subtract_partition_mean','sessN'});
            ...
            for roi = reg;
                ...
                for s = sn % for each subject
                    ...
                    for i = 1:size(partitions,1)
                        ...
                        if subtract_partition_mean
                            prepBetas{i} = bsxfun(@minus,t.betaUW{1}(partitionIdx,:),mean(t.betaUW{1}(partitionIdx,:)));
                        else
                            prepBetas{i} = t.betaUW{1}(partitionIdx,:);
                        end
    ...
    case 'pattern_reliability_acrossSess'
        ...
        fig = 1;
        sessN = 1;
        sess_combination = 'altogether'; % of 'sequential'
        subtract_partition_mean = 1; % subtract
        partitions = [1:2:numruns_task_sess; 2:2:numruns_task_sess];
        numRuns    = 1:numruns_task_sess;
        numConds   = num_seq;
        ...
        vararginoptions(varargin,{'roi','fig','sn','subtract_partition_mean','sessN','sess_combination'});
            ...
            for roi = reg;
                ...
                for s = sn % for each subject
                    ...
                    for c1 = numConds % for each condition
                        % condition mean activity pattern for this run partition
                        oddCon   = condIdx{1}==c1;
                        oddBetas1 = mean(prepBetas1{1}(oddCon,:));
                        oddBetas2 = mean(prepBetas2{1}(oddCon,:));
                        % condition mean activity pattern for the other run partition
                        evenCon   = condIdx{2}==c1;
                        evenBetas1 = mean(prepBetas1{2}(evenCon,:));
                        evenBetas2 = mean(prepBetas2{2}(evenCon,:));
                        ...
                        ceilingWitSess(c1) = sqrt(abs(tmpW1(1,2))*abs(tmpW2(1,2)));
          ...
          % gc2 = gca; hold on;
           for ss=1:length(sessN)
               for seq=1:2   % trained / untrained
                   ...
                   plot(gc1,beta(Stats.FoSEx==2 & Stats.sessN==ss & Stats.roi==roi(r)),beta(Stats.FoSEx==1 & Stats.sessN==ss & Stats.roi==roi(r)),mark_seq{seq},'Color',col_sess{ss},'linewidth',2); hold on; axis equal; hold on; grid on;
