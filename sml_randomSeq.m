...
baseDir = '/Users/nicola/Documents/Data/SuperMotorLearning/stimuli_struct';
cd (baseDir);
rng('default');
rng('shuffle');
load('AllTrip.mat');
...
switch(what)
    ...
    case 'RandomSeq_Memory'
        SeqAll = [Seq1;Seq2];
        
        for r = 1:numRandS
            good1=false;
            while  good1 == false % no two chunks repeated, no finger rep
                c = (datasample(AllChunksOrd,3,'Replace',false))';
                RandomSeq(r,:) = c(:);
                if sum(ismember(SeqAll,RandomSeq(r,:),'rows')) == 0 & (RandomSeq(r,1) == 1| RandomSeq(r,1) == 3|RandomSeq(r,1) == 5)
                    if (sum(ismember(diff(RandomSeq(r,:)),0))==0)
                        good1=1;
                    end
                else
        ...
        save('RandomSeq_Memory.mat','RandomSeq');
